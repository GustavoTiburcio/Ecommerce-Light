...
import Context, { ICarrinho, IContext, IEndUsu } from '../../context/Context';
...
export default function FinalizarCarrinho() {
  ...
  //Dados do comprador
  const [cgc, setCgc] = useState(dadosLogin?.cgc ?? '');
  const [nom, setNom] = useState(dadosLogin?.raz ?? '');
  const [email, setEmail] = useState(dadosLogin?.username ?? '');
  const [cel, setCel] = useState(dadosLogin?.fon ?? '');
