...
import Context, { IContext, IDadosLogin, IEndUsu } from '../../context/Context';
...
export default function PainelDeUsuario() {
  const navigate = useNavigate();
  const { itemMenu } = useParams();
  const { width } = useWindowDimensions();
  const isMobile = width <= 767;
  const { dadosLogin, setDadosLogin, configs }: IContext = useContext(Context);

  const [itemMenuSelecionado, setItemMenuSelecionado] = useState<string>(itemMenu || 'pedidos');
  ...
  //dados login
  const [dadLogin, setDadLogin] = useState<IDadosLogin>(dadosLogin);
  ...
  const itensMenu: Record<string, JSX.Element> = {
                  ...
                  <ProdutosDiv>
                    {detalhesPedidoSelecionado.itensPedido.map((item: any, index: number) => (
                      <ProdutosInfoDiv key={index}>
                        <span
                          onClick={() => navigate(`/produtoDetalhes/${item.codbar}/${item.mer.replaceAll(' ', '-')}`,
                            { state: { caminho: 'Home' } })
                          }
                        >
    ...
    enderecosDeEntrega:
      <>
        {dadLogin.endUsu.sort((a: IEndUsu, b: IEndUsu) => b.padent - a.padent).map((endUsu: IEndUsu, index: number) => (
  ...
  async function getPedidos() {
    try {
      const response = await api.get(`/pedidos/listarPedidoPorCliente?nome=${dadosLogin.username}`);
  ...
  async function postEndereco(endUsu: IEndUsu) {
    try {
      const payload = endUsu;
      payload['appuser'] = { id: dadosLogin.id };
  ...
  async function alterarSenha(e: any) {
    try {
      e.preventDefault();
      if (senhaAtual !== dadosLogin.password) {
      ...
      const novoDadosLogin = dadosLogin;
        ...
        {menuOptions.map((menu: IMenuOptions, index: number) => (
          <MenuButtons
            ...
            onClick={() => {
              ...
              navigate(`/painelDeUsuario/${menu.name}`);
          ...
          >
            &nbsp;&nbsp;<IconeDinamico nome={menu.iconName} size={20} />&nbsp;{menu.label}
          </MenuButtons>
        ))}
        <MenuButtons
          onClick={logout}>
          &nbsp;&nbsp;<IconeDinamico nome={'FaArrowLeft'} size={20} />&nbsp;Sair
        </MenuButtons>
      </>
    );
  }
  ...
  useEffect(() => {
    ...
    if (dadosLogin.password.startsWith('alt1@@')) {
      setSenhaAtual(dadosLogin.password);
    }
  ...
  return (
    <Container>
      <TitleContainer>
        <span>Olá <strong>{dadosLogin?.raz ?? ''}!!</strong> Acompanhe aqui seus pedidos e seus dados cadastrais.</span>
      </TitleContainer>
      <MenuContainer>
